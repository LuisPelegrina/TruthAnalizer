...
void test::AnalyzeEventsTruth::analyze(art::Event const& e) {
    ...
    //Access the generator information (MCTruth)
    //Create a Handle
    art::Handle <std::vector<simb::MCTruth>> MCT_handle;
    //Create a vector of MCTruth (for events with multiple Truth generator, in this case only 1 is expected)
    std::vector<art::Ptr < simb::MCTruth>> MCT_vec;
    //Get the MCTruth by using the label defined in "analysisConfigTruth.fcl"
    if (e.getByLabel(MCTruth_label, MCT_handle))
        art::fill_ptr_vector(MCT_vec, MCT_handle);

    //Save all the truth information of the generator
    ...
    //Access the geant4 information (MCParticles)
    //Create a Handle
    art::Handle<std::vector<simb::MCParticle>> MCP_handle;
    //Create a vector of MCParticles
    std::vector<art::Ptr<simb::MCParticle>> MCP_vec;
    //Get the MCParticles by using the label defined in "analysisConfigTruth.fcl"
